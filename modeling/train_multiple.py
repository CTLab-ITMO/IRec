...
def main():
    ...
    list_of_params = list(
        itertools.product(
            dataset_params, model_params, loss_function_params, optimizer_params
        )
    )
    ...
    for dataset_param, model_param, loss_param, optimizer_param in list_of_params[
        start_from:num
    ]:
        cnt += 1
        if cnt < start_from:
        ...
        eval_dataloader = BaseDataloader.create_from_config(
            config["dataloader"]["validation"], dataset=eval_sampler, **dataset.meta
        )
        ...
        utils.tensorboards.GLOBAL_TENSORBOARD_WRITER = (
            utils.tensorboards.TensorboardWriter(model_name, use_time=False)
        )
        ...
        eval_model = BaseModel.create_from_config(model_param, **dataset.meta).to(
            DEVICE
        )
