    ...
    def forward(self, inputs):
        ...
        if self.training:  # training mode
            ...
            positive_embeddings, positive_mask = create_masked_tensor(
                all_positive_embeddings, positive_lengths
            )
            negative_embeddings, negative_mask = create_masked_tensor(
                all_negative_embeddings, negative_lengths
            )

            positive_scores = torch.einsum(
                "bd,bsd->bs", user_embeddings, positive_embeddings
            )  # (batch_size, seq_len)
            negative_scores = torch.einsum(
                "bd,bsd->bs", user_embeddings, negative_embeddings
            )  # (batch_size, seq_len)
            ...
            candidate_scores = torch.einsum(
                "bd,nd->bn", user_embeddings, candidate_embeddings
