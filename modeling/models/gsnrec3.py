...
class GSNRec3Model(SequentialTorchModel, config_name='gsnrec3'):
    ...
    def __init__(
        ...
        self._mlp = nn.Linear(self._embedding_dim, self._embedding_dim, bias=True)
    ...
    def forward(self, inputs):
        ...
        if self.training:  # training mode
            ...
            # z_p siren
            all_graph_embeddings = torch.cat((graph_user_embeddings, graph_item_embeddings), dim=0)
            # shapes z_p:
            # [num_users + 2 + num_items + 2; embedding_dim]
            # например для датасета ml1m и num_layers = 2: [9069; 64], т.е. [6022 + 2 + 3043 + 2; 64]

            # print('graph_item_embeddings', graph_item_embeddings.shape)
            # print('negative_items_popularity', self._negative_items_popularity.shape)
            # print(self._negative_items_popularity)

            # list_minus_ids = inputs['item.negatives.ids'].tolist()
            # degree_minus_ids = dict([(i, list_minus_ids.count(i)) for i in range(self._num_items + 2)])
            # max_degree = max(degree_minus_ids.values())

            # minus_graph_item_embeddings = torch.mul(
            #     graph_item_embeddings,
            #     torch.tensor([[(degree_minus_ids[i] / max_degree) ** 0.5] for i in range(self._num_items + 2)]).to(DEVICE)
            # )

            minus_graph_item_embeddings = torch.mul(
                graph_item_embeddings,
                self._negative_items_popularity
            ).to(DEVICE)

            minus_graph_embeddings = torch.cat((graph_user_embeddings, minus_graph_item_embeddings), dim=0)

            x = F.dropout(F.relu(self._mlp(minus_graph_embeddings)), p=0.5, training=self.training)
            for i in range(1, self._mlp_layers):
                x = F.dropout(F.relu(self._mlp(x)), p=0.5, training=self.training)
            minus_graph_embeddings = x

            # print('minus_graph_embeddings', minus_graph_embeddings.shape)

            # # MLP part of siren
            # mlp = [self.E2]

            # #x = F.dropout(F.relu(self.mlps[0](self.E2)), p=0.5, training=self.training)
            # x = F.dropout(F.relu(self.mlps[0](minus_graph_embeddings)), p=0.5, training=self.training)
            # for i in range(1, self._mlp_layers):
            #     x = self.mlps[i](x)
            #     x = F.relu(x)
            ...
            # #print('mlp', len(mlp))
            # #print('mlp :', mlp)
            # minus_graph_embeddings = mlp[-1]
            # shapes z_n
            # [num_users + 2 + num_items + 2; embedding_dim]
            # например для датасета ml1m: [9069; 64]

            # Z
            w_p = self.q(F.dropout(torch.tanh((self.attn(all_graph_embeddings))), p=self._dropout_mlp, training=self.training))
            w_n = self.q(F.dropout(torch.tanh((self.attn(minus_graph_embeddings))), p=self._dropout_mlp, training=self.training))
            alpha_ = self.attn_softmax(torch.cat([w_p, w_n], dim=1))

            graph_embeddings = alpha_[:, 0].view(len(all_graph_embeddings), 1) * all_graph_embeddings - \
                               alpha_[:, 1].view(len(all_graph_embeddings), 1) * minus_graph_embeddings
            ...
            minus_positive_embeddings, _, minus_positive_mask = self._get_embeddings(
                inputs, self._positive_prefix, self._item_embeddings, minus_graph_embeddings
            )  # (batch_size, seq_len, embedding_dim)

            minus_graph_scores = torch.einsum(
                'bd,bsd->bs',
                user_embeddings,
                minus_positive_embeddings
            )  # (batch_size, seq_len)

            minus_graph_scores = minus_graph_scores[minus_positive_mask].unsqueeze(-1)  # (all_batch_events)
            ...
            return {
                ...
                'minus_scores': minus_graph_scores,
