...
class SasRecMCLSRModel(SequentialTorchModel, config_name='sasrec_mclsr'):
    ...
    def forward(self, inputs):
        ...
        if self.training:  # training mode
            ...
            # Graph downstream loss (option 0. without any loss) TODO test

            # Graph downstream loss (option 1. with existing negatives) [looks like trash] TODO test
            positive_graph_item_embeddings = \
                common_graph_item_embeddings[all_positive_sample_events]  # (all_batch_events, embedding_dim)
            negative_graph_item_embeddings = \
                common_graph_item_embeddings[all_negative_sample_events]  # (all_batch_events, embedding_dim)
            ...
            # Graph downstream loss (option 2. with negatives as other items) TODO test
            all_graph_scores = torch.einsum(
                'ad,bsd->abs', common_graph_user_embeddings, common_graph_item_embeddings
            ...
            # Downstream task
            # Option 1. Use shared downstream task
            # combined_representation = \
            #     self._alpha * all_sample_embeddings + \
            #     (1 - self._alpha) * common_graph_item_embeddings[mask]  # (all_batch_events, embedding_dim)

            # Option 2. Use downstream task only for sequential part
            combined_representation = all_sample_embeddings  # (all_batch_events, embedding_dim)
            ...
            return {
                ...
                # Graph task v1
                'user_graph_embeddings': common_graph_item_embeddings,
                'positive_graph_embeddings': positive_graph_item_embeddings,
                'negative_graph_embeddings': negative_graph_item_embeddings,

                # Graph task v2
                'graph_scores': all_graph_scores,
                'sequence_mask': mask,
