...
# class MCLSRModel(TorchModel, config_name='sasrec_mclsr'):
#     INF = 1e9
#
#     def __init__(
#             self,
...
#         # sequence_embeddings = sequence_embeddings[sequence_mask] + position_embedding[sequence_mask]
#         # sequence_embeddings, sequence_mask = create_masked_tensor(
#         #     data=sequence_embeddings, lengths=sequence_length
#         # )  # (batch_size, max_seq_len, embedding_dim), (batch_size, max_seq_len)
#
#         # encoder part
#         causal_mask = torch.tril(torch.ones(sequence_mask.shape[-1], sequence_mask.shape[-1])).bool().to(DEVICE)   # (seq_len, seq_len)
...
#
#             all_sample_embeddings = embeddings[sequence_mask]  # (all_batch_events, embedding_dim)
