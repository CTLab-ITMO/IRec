...
from utils import DEVICE, MetaParent, get_activation_function, maybe_to_list
...
class CompositeLoss(TorchLoss, config_name="composite"):
    ...
    def create_from_config(cls, config, **kwargs):
        ...
        return cls(
            losses=losses, weights=weights, output_prefix=config.get("output_prefix")
        )
...
class BinaryCrossEntropyLoss(TorchLoss, config_name="bce"):
    def __init__(
        self, predictions_prefix, labels_prefix, with_logits=True, output_prefix=None
    ):
        super().__init__()
        self._pred_prefix = predictions_prefix
        self._labels_prefix = labels_prefix
...
class SASRecRealLoss(TorchLoss, config_name="sasrec_real"):
    ...
    def forward(self, inputs):
        ...
        positive_loss = torch.log(nn.functional.sigmoid(positive_scores) + 1e-9)  # (x)
        negative_loss = torch.log(1.0 - nn.functional.sigmoid(negative_scores) + 1e-9)  # (x)
...
class SASRecLoss(TorchLoss, config_name="sasrec"):
    ...
    def forward(self, inputs):
        ...
        possible_indices = torch.arange(
            1, num_items + 1, device=negative_scores.device
        )  # 1, 2, ... num_items
        mask = torch.ones_like(
            possible_indices, dtype=torch.bool
        )  # True, True, ... True
        ...
        rand_idx = torch.randint(
            0,
            len(valid_indices),
            size=(negative_scores.shape[0], 1),
        ...
        negative_loss = torch.log(
            1.0 - nn.functional.sigmoid(negative_scores) + 1e-9
        ).sum(dim=-1)  # (x), added 1e-9 for Tiger baseline
        loss = positive_loss + negative_loss  # (x)
        loss = -loss.sum()  # (1)
...
class SamplesSoftmaxLoss(TorchLoss, config_name="sampled_softmax"):
    def __init__(
        self, queries_prefix, positive_prefix, negative_prefix, output_prefix=None
    ):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        positive_scores = torch.einsum(
            "bd,bd->b", queries_embeddings, positive_embeddings
...
class S3RecPretrainLoss(TorchLoss, config_name="s3rec_pretrain"):
    ...
    def forward(self, inputs):
        ...
        positive_scores = torch.einsum(
            "bd,bd->b", positive_embeddings, current_embeddings
        )  # (x)

        negative_scores = torch.einsum(
            "bd,bd->b", negative_embeddings, current_embeddings
        )  # (x)
        ...
        loss = torch.sum(
            self._criterion(distance, torch.ones_like(distance, dtype=torch.float32))
        )  # (1)
        if self._output_prefix is not None:
            inputs[self._output_prefix] = loss.cpu().item()
...
class Cl4sRecLoss(TorchLoss, config_name="cl4srec"):
    ...
    def forward(self, inputs):
        ...
        logits = torch.einsum(
            "bnd,bd->bn", all_items_representation, current_representation
...
class DuorecSSLLoss(TorchLoss, config_name="duorec_ssl"):
    ...
    def forward(self, inputs):
        ...
        dropout_loss = self._compute_partial_loss(
            original_embeddings, dropout_embeddings
        )
...
class MCLSRLoss(TorchLoss, config_name="mclsr"):
    ...
    def forward(self, inputs):
        ...
        negative_scores = torch.reshape(
            all_scores[~positive_mask], shape=(batch_size, batch_size - 1, seq_len)
