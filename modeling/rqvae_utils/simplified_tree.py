...
class SimplifiedTree:
    ...
    def build_tree_structure(
        ...
        residuals = (
            residuals.to(self.device).float()
            if sum_with_residuals
            else torch.zeros_like(residuals, device=self.device, dtype=torch.float)
        )
    ...
    def calculate_full(self, sem_ids: torch.Tensor) -> torch.Tensor:
        ...
        expanded_emb_table = self.embedding_table.unsqueeze(0).expand(
            sem_ids.shape[0], -1, -1, -1
        )  # (count, sem_id_len, codebook_size, emb_dim)

        index = (
            sem_ids.unsqueeze(-1).expand(-1, -1, self.emb_dim).unsqueeze(2)
        )  # (count, sem_id_len, 1, emb_dim)

        return (
            torch.gather(input=expanded_emb_table, index=index, dim=2).sum(1).squeeze(1)
        )  # (count, emb_dim)
    ...
    def query(self, request_sem_ids: torch.Tensor, items_to_query: int) -> torch.Tensor:
        ...
        request_embeddings = self.calculate_full(
            request_sem_ids
        )  # (batch_size, emb_dim)

        request_embeddings = request_embeddings.unsqueeze(1).expand(
            -1, self.sem_ids_count, -1
        )  # (batch_size, sem_ids_count, emb_dim)

        diff_norm = torch.norm(
            self.full_embeddings - request_embeddings, p=2, dim=2
        )  # (batch_size, sem_ids_count)

        indices = torch.argsort(diff_norm, descending=False, dim=1)[
            :, :items_to_query
        ]  # (batch_size, k)
    ...
    def _query(self, request_sem_ids: torch.Tensor, k: int) -> torch.Tensor:
        ...
        index = (
            request_sem_ids.unsqueeze(-1).expand(-1, -1, self.emb_dim).unsqueeze(2)
        )  # (batch_size, sem_id_len, 1, emb_dim)

        request_embeddings = torch.gather(
            input=self.embedding_table.unsqueeze(0).expand(
                request_sem_ids.shape[0], -1, -1, -1
            ),
            dim=2,
            index=index,
        ).sum(1)  # (batch_size, emb_dim)

        diff_norm = torch.cdist(
            self.full_embeddings, request_embeddings.unsqueeze(1), p=2
        ).squeeze(1)  # (batch_size, sem_ids_count)
