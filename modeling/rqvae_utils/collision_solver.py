...
class CollisionSolver:
    def __init__(
        ...
        """
        self._sem_ids_sparse_tensor: torch.Tensor = torch.empty(
            (0, 0)
        )  # тензор группирирующий остатки по semantic_id
        self.item_ids_sparse_tensor: torch.Tensor = torch.empty(
            (0, 0)
        )  # тензор группирирующий реальные айди айтемов по semantic_id
        self.counts_dict: dict[int, int] = defaultdict(
            int
        )  # тензор храняющий количество коллизий по semantic_id
        ...
        self.key: torch.Tensor = torch.tensor(
            [self.codebook_size**i for i in range(self.sem_id_len)],
            dtype=torch.long,
            device=self.device,
        )  # ключ для сопоставления числа каждому semantic_id

    def create_query_candidates_dict(
        self,
        ...
        unique_ids, inverse_indices, counts = torch.unique(
            unique_id, return_inverse=True, return_counts=True
        )
        sorted_indices = torch.argsort(
            inverse_indices
        )  # сортированные индексы чтобы совпадающие хэши шли подряд

        row_indices = inverse_indices[sorted_indices]  # отсортированные хэши

        offsets = torch.cumsum(counts, dim=0) - counts
        col_indices = (
            torch.arange(semantic_ids_count, device=self.device) - offsets[row_indices]
        )  # индексы от 0 до k внутри каждого набора из совпадающих хэшей

        indices = torch.stack(
            [unique_ids[row_indices], col_indices], dim=0
        )  # индексы для разреженного тензора: 1 размерность хэш, 2 размерность индексы от 0 до k для коллизий каждого хэша

        max_residuals_count = int(
            counts.max().item()
        )  # максимальное количество коллизий для одного sem_id
        max_sid = int(
            self.codebook_size**self.sem_id_len
        )  # максимальный хэш sem_id который может быть

        self._sem_ids_sparse_tensor = torch.sparse_coo_tensor(
            indices,
            residuals[sorted_indices],
            size=(max_sid, max_residuals_count, self.emb_dim),
            device=self.device,
        )  # (max_sid, max_residuals_count, emb_dim)

        self.counts_dict = defaultdict(
            int, zip(unique_ids.tolist(), counts.tolist())
        )  # sid -> collision count

        self.item_ids_sparse_tensor = torch.sparse_coo_tensor(
            indices,
            item_ids[sorted_indices],
            size=(max_sid, max_residuals_count),
            device=self.device,
            dtype=torch.int32,
        )  # (max_sid, max_residuals_count)

    def get_residuals_by_semantic_id_batch(
        ...
        candidates = torch.stack(
            [self._sem_ids_sparse_tensor[key].to_dense() for key in unique_ids]
        )
        counts = torch.tensor(
            [self.counts_dict[key.item()] for key in unique_ids], device=self.device
        )
        mask = torch.arange(candidates.shape[1], device=self.device).expand(
            len(unique_ids), -1
        ) < counts.view(-1, 1)
    ...
    def get_pred_scores(
        self, semantic_ids: torch.Tensor, pred_residuals: torch.Tensor
        ...
        pred_scores = torch.einsum(
            "njk,nk->nj", candidates, pred_residuals
        ).masked_fill(~mask, -torch.inf)
        pred_indices = torch.argmax(pred_scores, dim=1)
        pred_item_ids = torch.stack(
            [
                self.item_ids_sparse_tensor[unique_ids[i]][pred_indices[i]]
    ...
    def get_true_dedup_tokens(
        self, semantic_ids: torch.Tensor, true_residuals: torch.Tensor
    ) -> dict[str, torch.Tensor]:
        """
        :param semantic_id: [batch_size, sem_id_len] semantic ids (без токена решающего коллизии)
        ...
        return {"true_dedup_tokens": true_dedup_tokens}

    def get_item_ids_batch(
        self, semantic_ids: torch.Tensor, dedup_tokens: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        item_ids = torch.stack(
            [
                self.item_ids_sparse_tensor[unique_ids[i]][dedup_tokens[i]]
                for i in range(semantic_ids.shape[0])
            ]
        )
