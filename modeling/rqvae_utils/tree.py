...
class Tree:
    def __init__(self, embedding_table: torch.Tensor, device: torch.device = DEVICE):
        ...
        self.embedding_table: torch.Tensor = (
            embedding_table  # (semantic_id_len, codebook_size, emb_dim)
        )
    ...
    def build_tree_structure(
        ...
        self.key = torch.tensor(
            [self.codebook_size**i for i in range(self.sem_id_len - 1, -1, -1)],
            dtype=torch.long,
            device=self.device,
        )
        ...
        result = torch.full(
            size=[self.codebook_size**self.sem_id_len],
            fill_value=0,
            dtype=torch.int64,
            device=self.device,
        )
        temp_unique_id = self.sids * self.codebook_size
        temp_sem_ids = torch.concat(
            [
                semantic_ids,
                torch.zeros(self.sem_ids_count, device=self.device).unsqueeze(1),
            ],
            dim=-1,
        )
        ...
        for i in range(0, self.sem_id_len + 1):
            temp_unique_id = (
                temp_unique_id
                - (self.codebook_size**i) * temp_sem_ids[:, self.sem_id_len - i]
            )
            temp_unique_ids, temp_inverse_indices = torch.unique(
                temp_unique_id, return_inverse=True
            )
            temp_counts = torch.bincount(temp_inverse_indices)
            truncated_ids = torch.floor_divide(
                input=temp_unique_id, other=(self.codebook_size ** (i + 1))
            ).long()
    ...
    def get_counts(self, sem_ids: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        mask_sem = (
            i < (self.sem_id_len - offsets.unsqueeze(1))
        ).long()  # (sem_id_len + 1, sem_id_len)
        divs = torch.pow(self.codebook_size, offsets)  # (sem_id_len + 1,)

        C = (
            sem_ids.unsqueeze(1)
            * mask_sem.unsqueeze(0)
            * self.key.unsqueeze(0).unsqueeze(1)
        ).sum(dim=-1)
        B = C // divs.unsqueeze(0)

        return C, self.A[
            B
        ]  # (batch_size, sem_id_len + 1), (batch_size, sem_id_len + 1)
    ...
    def calc_ol(
        self, batch_ids: torch.Tensor, k: int
    ...
    def calc_il(self, batch_ids, k):
        ...
        c, a = self.get_counts(batch_ids)
        extended_c = torch.concat(
            [torch.tensor(float("inf"), device=self.device).expand(batch_dim, 1), c],
            dim=1,
        )

        il = torch.argmax((a > k).long(), dim=-1) - 1  # (bs,)
        gather_il = torch.gather(
            extended_c, dim=1, index=(il + 1).unsqueeze(1)
        ).squeeze()  # (bs,)
    ...
    def get_request_embeddings(
        self, decomposed_embeddings: torch.Tensor, levels: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        mask = torch.arange(1, self.sem_id_len + 2, device=self.device) >= torch.arange(
            self.sem_id_len + 2, 0, -1, device=self.device
        ).unsqueeze(1)
        return torch.sum(
            decomposed_embeddings * mask[levels + 1].unsqueeze(-1), dim=1
        )  # (bs, emb_dim)

    def calculate_full(
        self, sem_ids: torch.Tensor, residuals: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        embs = torch.gather(
            input=self.embedding_table.unsqueeze(0).expand(count, -1, -1, -1),
            dim=2,
            index=index,
        )  # expand бесплатный по памяти
        decomposed_embs = torch.concat(
            [embs.squeeze(2), residuals.unsqueeze(1)], dim=1
        )  # (sem_ids_count, emb_dim)

        assert decomposed_embs.shape == (
            sem_ids.shape[0],
            self.sem_id_len + 1,
            self.emb_dim,
        )
    ...
    def calculate_level_embeddings(
        self, decomposed_embeddings: torch.Tensor, levels: torch.Tensor
    ) -> torch.Tensor:
        """
        :param decomposed_embeddings: разложение sem_ids на эмбеддинги (count, sem_id_len +1, emb_dim)
        ...
        """
        assert decomposed_embeddings.shape == (
            self.sem_ids_count,
            self.sem_id_len + 1,
            self.emb_dim,
        )
        ...
        mask = (
            torch.arange(1, self.sem_id_len + 2, device=self.device)
            >= torch.arange(self.sem_id_len + 2, 0, -1, device=self.device).unsqueeze(1)
        ).float()
        sids_mask = mask[levels + 1].unsqueeze(-1)  # (batch_size, sem_id_len + 1, 1)
        return torch.einsum(
            "nld,bld->bnd", decomposed_embeddings, sids_mask
        )  # (batch_size, sem_ids_count, emb_dim)

    def mask_result(self, result: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        return torch.where(
            mask, result, torch.tensor(float("-inf"), device=self.device)
        )

    def query(
        self,
        request_sem_ids: torch.Tensor,
        ...
        ol_scores = (
            torch.matmul(ol_sids_embeddings, ol_request_embeddings.unsqueeze(-1))
            .squeeze(-1)
            .detach()
            .cpu()
        )
        ...
        il_scores = (
            torch.matmul(il_sids_embeddings, il_request_embeddings.unsqueeze(-1))
            .squeeze(-1)
            .detach()
            .cpu()
        )
        ...
        ids = np.lexsort(
            keys=(
                -torch.cat([il_scores, ol_scores], dim=1),
                ~torch.cat(
                    [torch.ones_like(il_mask), torch.zeros_like(ol_mask)], dim=1
                ),
                ~torch.cat([il_mask, ol_mask], dim=1),
            )
        )

        ids = (ids % self.sem_ids_count)[:, : self.sem_ids_count][
            :, :items_to_query
        ]  # (batch_size, k)
