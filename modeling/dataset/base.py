...
# TODO скопировать весь этот код и везде где надо вызывать код от positive_domain, как в siren
...
class NegativeRatingsGraphDataset(BaseDataset, config_name='negative_ratings_graph'):
...
class NegativeRatingsScientificDataset(ScientificDataset, config_name='negative_rating'):
    def __init__(
            self,
            train_sampler,
            validation_sampler,
    ...
    def create_from_config(cls, config, **kwargs):
        ...
        domains = [positive_domain, negative_domain]
        max_sequence_length = config['max_sequence_length']
        max_user_idx, max_item_idx = 0, 0
        ...
        for domain in domains:
            ...
            with open(dataset_path, 'r') as f:
                for user_items_info, ratings in zip_longest(*[f] * 2):
                    ...
                    assert len(item_ids) >= 5
                    assert len(ratings) == len(item_ids)
