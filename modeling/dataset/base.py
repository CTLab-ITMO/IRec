...
from dataset.samplers import EvalSampler, TrainSampler
...
class SequenceDataset(BaseDataset, config_name="sequence"):
    ...
    @classmethod
    def _create_dataset(
        cls, dir_path, part, max_sequence_length=None, use_cached=False
    ):
        max_user_id = 0
        max_item_id = 0
        max_sequence_len = 0
        ...
        if use_cached and os.path.exists(os.path.join(dir_path, "{}.pkl".format(part))):
            ...
            with open(
                os.path.join(dir_path, "{}.pkl".format(part)), "rb"
            ) as dataset_file:
                dataset, max_user_id, max_item_id, max_sequence_len = pickle.load(
                    dataset_file
                )
        else:
            logger.info(
            ...
            logger.info(
                "{} dataset max sequence length: {}".format(part, max_sequence_len)
            )

            with open(
                os.path.join(dir_path, "{}.pkl".format(part)), "wb"
            ) as dataset_file:
                pickle.dump(
