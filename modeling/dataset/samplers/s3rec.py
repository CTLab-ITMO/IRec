...
class S3RecPretrainTrainSampler(TrainSampler, config_name="s3rec_pretrain"):
    ...
    def __getitem__(self, index):
        ...
        negative_sequence[-1] = self._negative_sampler.generate_negative_samples(
            sample, 1
        )[0]
        assert (
            len(positive_sequence)
            == len(negative_sequence)
            == len(masked_sequence)
            == len(item_sequence)
        )
        ...
        negative_start_id = np.random.randint(
            0, len(self._long_sequence) - sample_length
        )
        masked_segment_sequence = (
            item_sequence[:start_id]
            + [self._mask_item_idx] * sample_length
            + item_sequence[start_id + sample_length :]
        )
        positive_segment = item_sequence[start_id : start_id + sample_length]
        negative_segment = self._long_sequence[
            negative_start_id : negative_start_id + sample_length
        ]
