...
class MCLSRModel(TorchModel, config_name='mclsr'):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            negative_ids = negative_ids.view(batch_size, -1)
            negative_embeddings = self._item_embeddings(negative_ids) # (batch_size, num_negatives, embedding_dim)

            # import code; code.interact(local=locals())

            logq_users = self.log_user_freq[user_ids]
            labels_flat = labels.view(-1)
            log_q_pos = self.log_item_freq[labels_flat].unsqueeze(1)     # (B, 1)
            log_q_neg = self.log_item_freq[negative_ids]            # (B, N)
            assert log_q_pos.dim() == 2 and log_q_neg.dim() == 2, \
                f"log_q_pos.shape={log_q_pos.shape}, log_q_neg.shape={log_q_neg.shape}"

            log_q_items_softmax = torch.cat([log_q_pos, log_q_neg], dim=1)  # (B, 1 + N)
            log_q_items_cl = self.log_item_freq[unique_item_ids]
            ...
            return {
                ...
                'log_q_users': logq_users, # (B,)

                'log_q_items_softmax': log_q_items_softmax,  # (B, 1 + N)

                'log_q_items_cl': log_q_items_cl,     # (num_unique_items,)
