...
EXPERIMENT_NAME = 'rqvae_beauty_0'
IREC_PATH = '../../'
...
def main():
    ...
    dataset = EmbeddingDataset(
        data_path=os.path.join(IREC_PATH, 'data/Beauty/content_embeddings_0_part.pkl')
    )
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,
        drop_last=True,
    ).map(Collate()).map(ToTorch()).map(ToDevice(DEVICE)).map(ProcessEmbeddings(embedding_dim=INPUT_DIM, keys=['embedding'])).repeat(NUM_EPOCHS)
