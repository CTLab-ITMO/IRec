    ...
    def forward(self, inputs):
        ...
        if not self.training:
            ...
            for k in [5, 10, 20]:
                ...
                # for prefix_length in range(1, self._sem_id_len + 1):
                #     metrics[f'correct_prefix_{prefix_length}_recall@{self._num_return_sequences}'] = (
                #         torch.eq(predictions[:, :k, :prefix_length], labels[:, None, :prefix_length]).long().sum(dim=-1) == prefix_length
                #     ).float().sum(dim=-1).mean(dim=-1).item() / self._num_return_sequences
